...
class SearchResource extends BaseTypesenseResource_1.BaseTypesenseResource {
    constructor() {
        super('search');
    }
    getOperations() {
    ...
    async performSearch(context, client, itemIndex) {
    ...
    async performMultiSearch(context, client, itemIndex) {
    ...
    async performSearchWithConversation(context, client, itemIndex) {
        const collection = this.validateRequired(context, 'collection', itemIndex);
        const query = this.validateRequired(context, 'query', itemIndex);
    ...
    buildSearchParameters(context, itemIndex) {
        const params = {};
    ...
    processAdvancedSearchParameters(params) {
        const processed = {};
        if (params.prefix) {
    ...
    async performVectorSearch(context, client, itemIndex) {
        const collection = this.validateRequired(context, 'collection', itemIndex);
        ...
        const vector = vectorQuery.split(',').map((v) => parseFloat(v.trim()));
        if (vector.some(isNaN)) {
            throw new n8n_workflow_1.NodeOperationError(context.getNode(), 'Vector query must be comma-separated numbers.', { itemIndex });
        }
        const searchParams = {
    ...
    async performSemanticSearch(context, client, itemIndex) {
        const collection = this.validateRequired(context, 'collection', itemIndex);
    ...
    async performAdvancedSearch(context, client, itemIndex) {
        const collection = this.validateRequired(context, 'collection', itemIndex);
        const query = this.validateRequired(context, 'query', itemIndex);
        const searchParams = this.buildSearchParameters(context, itemIndex);
    ...
    processAdvancedQueryParameters(params) {
        const processed = {};
